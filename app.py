import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd
from datetime import datetime

# --- CONFIGURACIÓN DE PÁGINA ---
st.set_page_config(page_title="Inmobiliaria Pro v46", layout="wide")

# --- CREDENCIALES INYECTADAS (Plan B para ignorar Secrets) ---
creds = {
    "type": "service_account",
    "project_id": "inmobiliaria-487222",
    "private_key_id": "5e3fa80e51ca83ba8c1fcc4d56347400ec45816f",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "inmobiliaria@inmobiliaria-487222.iam.gserviceaccount.com",
    "client_id": "118429869319608246472",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/inmobiliaria%40inmobiliaria-487222.iam.gserviceaccount.com"
}

URL_SHEET = "https://docs.google.com/spreadsheets/d/1TIeJ2fjJ6WHn124b8iL9LgTNuRZ_50YyekSad0uK1jE/"

# --- CONEXIÓN DIRECTA ---
try:
    # Usamos **creds para pasar el diccionario y evitamos el doble 'type'
    conn = st.connection("gsheets", type=GSheetsConnection, **creds)
except Exception as e:
    st.error(f"Error de conexión crítica: {e}")
    st.stop()

# --- FUNCIONES ---
def leer(pestana):
    return conn.read(spreadsheet=URL_SHEET, worksheet=pestana)

def guardar(df, pestana):
    conn.update(spreadsheet=URL_SHEET, worksheet=pestana, data=df)
    st.cache_data.clear()

# --- SEGURIDAD ---
if "auth" not in st.session_state: st.session_state.auth = False
if not st.session_state.auth:
    pw = st.text_input("Clave:", type="password")
    if st.button("Entrar"):
        if pw == "Terrenos2026":
            st.session_state.auth = True
            st.rerun()
    st.stop()

# --- INTERFAZ ---
menu = st.sidebar.radio("Menú", ["Resumen", "Ubicaciones", "Pagos"])

if menu == "Ubicaciones":
    st.header("📍 Gestión de Terrenos")
    df = leer("terrenos")
    
    with st.form("nuevo"):
        c1, c2, c3 = st.columns(3)
        m = c1.text_input("Manzana")
        l = c2.text_input("Lote")
        p = c3.number_input("Precio", min_value=0.0)
        if st.form_submit_button("Guardar en Drive"):
            nueva_fila = pd.DataFrame([{"manzana": m, "lote": l, "costo": p, "estatus": "Disponible"}])
            df_final = pd.concat([df, nueva_fila], ignore_index=True)
            guardar(df_final, "terrenos")
            st.success("¡Datos sincronizados con Google Sheets!")
            st.rerun()
    st.dataframe(df, use_container_width=True)

elif menu == "Resumen":
    st.header("📋 Vista General")
    df_t = leer("terrenos")
    if not df_t.empty:
        st.metric("Total Lotes", len(df_t))
        st.table(df_t)
    else:
        st.info("No hay datos en 'terrenos'.")
